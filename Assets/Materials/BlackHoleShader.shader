...
Shader "DotCrossDot/BlackHoleRaymarching"
{
		...
		SubShader
	{
		...
		Pass
		{
			...
			float sdSphere(float3 p, float s, float3 bhCenter)
			{
				return length(p.xyz - bhCenter.xyz) - s;
			}

			float sdRoundedCylinder(float3 p, float ra, float rb, float h, float3 bhCenter)
			{
				float2 d = float2(length(p.xz - bhCenter.xz) - 2.0 * ra + rb, abs(p.y) - h);
			...
			float accretionDiskSDF(float3 p, float3 bhCenter) {
				float p1 = sdRoundedCylinder(p, 3.5, 0.25, 0.01, bhCenter);
				float p2 = sdSphere(p, 3.5, bhCenter);
			...
			fixed4 raymarch(float3 ro, float3 rd) {
				...
				float3 blackHolePosition2 = float3(5, 0, 0);
				...
				for (int i = 0; i < maxstep; ++i) {
					...
					newRayDir += normalize(lerp(unaffectedDir, maxAffectedDir2, lerpValue2)) * stepSize; // 2nd hizzle

					// Move the lightray along and calculate the sdf result
					float3 newPos = previousPos + newRayDir;

					float sdfResult = accretionDiskSDF(newPos, blackHolePosition);
					float sdfResult2 = accretionDiskSDF(newPos, blackHolePosition2);
